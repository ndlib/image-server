...
RUN sed -e 's+home\/myself\/images+imageroot+' -e 's/#cache.server/cache.server/' < cantaloupe.properties.sample > ctlp.props \
  ...
  &&    sed -i "s/S3Source.BasicLookupStrategy.bucket.name =/S3Source.BasicLookupStrategy.bucket.name = iiif-image-storage/g" ctlp.props\
  ...
  &&    sed -i "\$a AmazonS3Resolver.BasicLookupStrategy.bucket.name = iiif-image-storage" ctlp.props\
...
EXPOSE 8182
CMD ["java", "-Dcantaloupe.config=ctlp.props", "-Xmx2g", "-jar", "cantaloupe.war"]
